...
## Thor encode (FACT, datasheet DS-11945-001 table 2-4 / 2-5)

YUV420 8-bit, indicative. NVENC shares the GPU rail.

| SKU | NVENC | HEVC HQ 4Kp30 | HEVC HQ 4Kp60 | HEVC UHP 4Kp30 | HEVC UHP 4Kp60 |
|---|---|---|---|---|---|
| **T4000** 70 W | 1× | **2** | **1** | 6 | 3 |
| **T5000** 120 W | 2× | ~double T4000 HQ | marketing **6× 4Kp60** is UHP-class, not HQ | | |

AGX Thor Developer Kit = **T5000 module** (NVIDIA spec table 2026-09-01). There is no T4000 kit.
...
| 25GbE (T4000 has 3×, T5000 4×, AGX QSFP28 = 4×25) | 25 Gbps | ~23 Gbps | fiber/DAC | 4 uncompressed 4K30, or 2× 4K60 10-bit 4:2:2. |
| QSFP28 / 100GbE | 100 Gbps | ~90 Gbps | fiber | Whole truck → NAS. Also HSB camera path on the AGX kit. |
...
| 1+6 4K60 | ~70 Gbps | ~1.0 Gbps | T4000 no. T5000 UHP marketing yes, HQ no. | 5GbE still yes for HEVC | **100GbE / QSFP** |

USB-C is never the answer for the uncompressed column. Fiber or QSFP28 is the truck→NAS pipe once more than one uncompressed 4K exists. For HEVC satellites, **gigabit PoE + 5GbE/10GbE aggregation** is plenty.

## Hybrid working plan

```
[body cam] --CSI/GMSL--> Thor NVENC --> NVMe ring --> 10/25/100GbE --> NAS
...
- NVIDIA Jetson Thor spec table (AGX kit = T5000; T4000 1× NVENC, 3×25GbE; T5000 2× NVENC, 4×25GbE): https://www.nvidia.com/en-us/autonomous-machines/embedded-systems/jetson-thor/
- Module datasheet DS-11945-001 encode tables 2-4 / 2-5 (YUV420 8-bit indicative)
