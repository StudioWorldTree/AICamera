...
| Box | Role | Residency |
|---|---|---|
| Thor T4000 | Production body. 64 GB unified, 70 W, 1× NVENC. Pack `t4000`. | All live slots **resident**. SMALL weights. 30 fps design target. |
| fractal1 3090 | Local pipe. FP16, 24 GB + 16 GB host. Envelope `3090`. Not a T4000. | **Swap** one SMALL slot at a time. Slow on purpose. |
| RunPod RTX PRO 6000 Blackwell Server 96 GB | 30 fps all-perception-filters lab. Same family as the truck quality box (1792 GB/s). | All live slots **resident**. LARGE weights. |
| Truck 6000 | Quality box. DiT, gsplat, 27B overnight. Not the body. | Not the live mix. |
...
One pipeline. Residency is config (`resident` | `swap`). Pack is the
envelope id. Cartridge catalog (`add-cartridge-model`) carries per-box
costs; this change is the envelope list those columns mean. RunPod and
truck 6000 share the `6000` quality-class column until measured apart.
